...
function Result() {
  ...
  return (
    ...
    text- p-0.5 mt-10 rounded-full'>
      ...
      max-sm:w-20 placeholder-color w-2'/>
      <button type='submit' className='bg-zinc-900 px-10 sm:px-16 rounded-full text-white'>Generate</button>
    </div>
}
{isImageLoaded && 

    <div className='flex gap-2 flex-wrap justify-center text-white
    text-sm p-0.5 mt-10 rounded-full cursor-pointer'>
      <p onClick={()=>{setIsImageLoaded(false)}}  className='bg-transparent border border-zinc-900 
       text-black px-8 py-3 rounded-full cursor-pointer'>Genrate another</p>
